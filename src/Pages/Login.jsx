...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!/^(?=.*?[A-Z])(?=.*?[#?!@$%^&*-]).{6,}/.test(password)) {
      return toast.error(
        "Password Should be At lest 6, one Capital and special!",
        {
          position: "top-center",
          autoClose: 5000,
      ...
      .catch((error) => {
        const Emessage =
          error.message == "Firebase: Error (auth/invalid-login-credentials)."
            ? "Your E-mail or Password incorrect"
            : error.message;
